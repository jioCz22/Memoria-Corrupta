...
/* --- MODO DE REVISIÓN DE PARES (DEBUG) --- */
document.addEventListener('DOMContentLoaded', () => {
    const reviewBtn = document.getElementById('peer-review-btn');
    const gameBoard = document.getElementById('game-board');

    if (reviewBtn && gameBoard) {
        reviewBtn.addEventListener('click', () => {
            // Alternamos la clase en el contenedor del tablero
            gameBoard.classList.toggle('peer-review-mode');
            
            // Alternamos el estilo del botón
            reviewBtn.classList.toggle('active');

            // Cambiamos el texto del botón para indicar el estado
            if (gameBoard.classList.contains('peer-review-mode')) {
                reviewBtn.innerHTML = '🕶️ Ocultar Par';
            } else {
                reviewBtn.innerHTML = '👁️ Revisión Par';
            }
        });
    }
});